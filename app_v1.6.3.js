...
*/
    ...
    redis_get(rclient, key, keys, keys_origin, keys_origink, unit_type, acc_type, function(err, data){
        ...
        if(type_avg_speed) {
            ...
            var result1 = [];
            var typeLabel;
            var resultAfterGrp = _.groupBy(result, function(re){
                var type = re.type;
                if(/speed_request/.test(type)) {
                    return type.replace('speed_request','');
                } else if (/request_count_2xx/.test(type)) {
                    return type.replace('request_count_2xx','');
                }
            });
            _.each(resultAfterGrp, function(re){
                _.each(re, function(mo){
                    var type = mo.type;
                    if(/speed_request/.test(type)) {
                    //if(mo.type == 'speed_request')
                        t1 = mo.value;
                        typeLabel = type.replace('speed_request','avg_speed');
                ...
                result1.push({type: typeLabel, value: vva});
            });
            result = result1;
        }
        if(type_traffic_ps) {
            ...
            _.each(result, function(mo){
                ...
                if(/body_bytes_sent/.test(type)) {
                    ...
                    result1.push({type: type.replace('body_bytes_sent','traffic_ps'), value: vva});
                } else {
                    result1.push(mo);
                }
            })
...
server.listen(8001, "113.164.27.58",function () {
});