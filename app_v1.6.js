...
function redis_reget(rclient,key, unit_key, unit_type, level, acc_type, force){
	...
	rclient.hget(key, mkey, function(err, rres){
		winston.log("debug", "level:" + level + "--> hget:" + key + " -> " + mkey + " ret:" + rres );
...
//                if(rres || rres == 0) {
		...
		var curt =  moment().format('YYYYMMDDHHmm');
			...
			if(timekey <= curt) {
				...
				var val = 0;
				if(force || newl <= 1)
	                    		val = redis_reget(rclient, key, unit_key + jj, unit_type1, newl, acc_type);
				else {
					console.log("queue process");
					queue_process.push([key, unit_key, unit_type, acc_type]);
				}
	                    	datar.push(val);
			}
                 }
			...
			//winston.log('debug', 'acc_type ' + acc_type + ' s:' +  s);
                	if(unit_key != uu) {
				if(unit_type != 'MINUTE') { 
...
//	winston.log('debug', 'tt:' + tt);
	if(/traffic_ps/.test(tt)) {
	...
	//winston.log('debug', 'acc_type:' + acc_type);
})
		...
		if(err) {
			...
			if(acc_type == 'max') {
				for(var ii = 0; ii < data.length; ii ++ ) {
					var ee = data[ii];
					var kk = type  + '|' + ee['name'];
					var ttk = parseInt(ee['value']);
					if(totali[kk] && ttk > totali[kk]) {
						totali[kk] = ttk;
						winston.log('debug', 'acc_type ' + acc_type + ' totali[' + kk + ']' + 'val:' +  totali[kk]);
					}
				}
			...
			_.each(result, function(mo){
				...
				if(/body_bytes_sent/.test(type)) {
					var vva = _.map(mo.value, function(kkv){
						kkv['value'] = bw2traf(kkv['value'], 'minute');
						return kkv;
					})
				...
				if(/body_bytes_sent/.test(tt)) {
 					if(type_traffic_ps) {
						if(!ddata['traffic_ps']) ddata['traffic_ps'] = [];
					   	ddata['traffic_ps'].push({name: dd, value: bw2traf(totali[kk],'minute')});
 					} else {
...
//				}
		...
		Object.keys(ddata).forEach(function(kk){
			total.push({type: kk, value: ddata[kk]});	
		})
...
var myworker = new Worker(function(){
 	this.onmessage = function (event) {
		...
		redis_reget(rclient, args[0], args[1], args[2], 0, args[3], true)	
		}
	}
})
function process_queue(myworker){
	console.log("start process");
	while(queue_process.length > 0) {
		var args = queue_process.shift();
		if(args) {
			myworker.postMessage(JSON.stringify(args));
		//	newq.push(redis_reget(rclient1, args[0], args[1], args[2], 0, args[3], true));
		}
	}
}
(function run_process(){
	var mytimeout = setTimeout(function(){
		process_queue(myworker);
